...
import { DbResponseParser } from './patched-response-parser';
        ...
        try {
            ...
            if (analysis) {
                try {
                    ...
                    debug('[proxy] Analysis saved successfully');
                } catch (saveErr) {
                    console.error('[proxy] Error saving analysis:', saveErr);
                }
            }
        } catch (error) {
...
const listener = net.createServer((rawClient) => {
    ...
    const handleFirstData = (chunk: Buffer) => {
        if (hasReceivedData) return;

        buffered = Buffer.concat([buffered, chunk]);
        if (buffered.length < 8) {
            debug('[proxy] Buffering handshake data, current length:', buffered.length);
            return;
        }

        hasReceivedData = true;
        rawClient.removeListener('data', handleFirstData);

        const first8 = buffered;
        ...
        const isCancelRequest = first8.length >= 8 && first8.readUInt32BE(4) === 0x04d2162e;
        ...
        if (isSSLRequest) {
            ...
            debug('[proxy] Detected CancelRequest, forwarding blindly without parsing');
            // CancelRequest is a special message that opens a new connection, sends the request, and closes.
            // pg-server parser might not handle it as a startup message, so we just forward it blindly.
            const dbSock = createBackendSocket();
            dbSock.on('error', (e) => console.error('[proxy] Backend connection error (CancelRequest):', e));

            // Write the buffered data
            dbSock.write(first8);

            // Pipe the rest
            rawClient.pipe(dbSock).pipe(rawClient);
        } else {
